...
where they become one. They are never required for the coordinates.

---

## Coordinates

**There is exactly one set.** The Zieldaten are reconstructed at the end of
Chapter 8, regardless of how many fragments were found.

Chapter 9 does not hand out a second target. The remainder of the transmission
names the *same* place — which is the whole point of it: it is the only part of
the target that does not come from R-3MI and V-TGM, so it is what tells the
player the two of them did not invent it. Mechanically it is the same string
with a different label (`ZIELDATEN · EXTERN BESTÄTIGT`); narratively it is
corroboration, and for the player it is a completionist flourish rather than a
second prize.

The set is not a readable string in the source. It lives in
`GameEngine.calibration` as shifted fragments that are put back together in the
order the finished reconstruction produces. This is not security — static
JavaScript never is — it only keeps the answer from falling out of a search.
**`COORDS.md` is the one document you need before release.**
