...
| Ch9 — BONUS (hidden) | outside curve | done | Access-gated on **Chapter 8 complete AND all 5 Signalnischen** — a direct URL shows an in-universe denial with a `[ SIGNALARCHIV ]` route and leaks nothing. A narrative chamber: an archive of past "projects" (Italian Brainrot/FRIGO → `italian_brainrot`, Crypto Colors, The Transmission) narrated by R-3MI/V-TGM with knowledge they shouldn't have. After all 3 exhibits, a glitching anomaly hotspot triggers the reveal — the player was only ever useful, the calibration was a rebuild, and the two are now the admins. Dialogue box shakes & bleeds red, portraits turn sinister; unlocks `bonus_found`. Entered from the `[ ??? BETRETEN ]` button that fades in under the normal buttons on the Ch8 end screen — Chapter 8 never names it, never calls it a bonus and never promises a second reward. The hook is the unregistered chamber, nothing else |
