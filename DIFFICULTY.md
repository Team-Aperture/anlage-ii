...
| Ch2 — Wartungsgarten      | **5.25** | done | P1 Tau-Sequenz ~5 (order forced by the ambient shift; exactly one plant order works) · P2 Frostmuster ~4.5–5 (672 ways to cut six 4-cell groups; the 6 pre-carved walls leave 9, and the 18-channel ice budget — pink ones included, stated on the card and by F-RØ5CHI — leaves **exactly one**, which uses all 18; running out of ice says whether the grouping would have needed more, or a channel separates nothing). One shared 4-step hint ladder per puzzle (`HINT_MAX` in `chapter2.js`) — observation → relationship → method → last resort, voiced by whichever of the three you ask |
...
1. **Hint budget.** Most direct lever. A ladder is walked once per chapter run:
   each puzzle window keeps its position through closing, reopening, retries
   and reloads (stored in the checkpoint as `hints: { ladder: stepsUsed }`);
   only a new stage (Ch3) or phase (Ch6), or a fresh run, starts a new one.
   Ch1 = one shared 3-step ladder per puzzle
...
4. **Problem size / constraints.** Ch2 Frostmuster: 5×5 → six connected
   4-cell groups + isolated well. There are 672 such groupings; the 6
   pre-carved `FROST_FIXED` walls leave 9, and the 18-channel cap (ice =
   22 − number of 2×2 squares, so 18 is also the minimum any grouping needs)
   leaves **exactly one** (`tests/analyse_ch2_frost.js`). The budget is a
   stated rule, not a hidden one: the card, F-RØ5CHI's intro and hint 2 name
   it, hint 3 says why squares save ice.
...
10. **Slack in a budget.** Ch5's 14-G gives exactly the pressure the two
   required systems need (`buildSupply` derives the budget from them) and
   every pressure line costs the same, so the reserve says *how many* systems
   can run (two) but not *which* — only the schema does. (With unequal costs
   the one reserve-emptying set was the answer, so "spend it all" solved it
   unread.) Widening the budget by one
